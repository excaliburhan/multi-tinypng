...
var setOption = function(param, value) {
  if(typeof param !== 'undefined' && typeof value !== 'undefined') {
    if (param === 'api_keys') {
      var apiKeys = options[param] || [];
      if (options[param].indexOf(value) === -1) {
        apiKeys.push(value);
        options[param] = apiKeys;
      }
    } else {
 ...
 */
var switchKeys = function() {
  var apiKeys = options.api_keys || [];
  var apiKey = options.api_key || '';
  if (!apiKeys.length || !apiKey) {
    return;
  } else {
    var index = apiKeys.indexOf(apiKey);
    if (index + 1 === apiKeys.length) { // its the last key
      apiKey = apiKeys[0];
    } else {
      apiKey = apiKeys[index + 1];
    }
